...
The most common route, referred to as "root" route is usually defined with a signle forward-slash `/`. This is the home page of most websites. If you think of a static website of just HTML files, the root would be defined by an `index.html` file. 
...
That's how you create multiple pages! You can add a few more routes just to try it out. You may notice now why it might make sense to use multiple files in larger projects. For websites with a lots of pages, a single app.py file might get a little full. But you'll learn more about that later.
...
It's not necessarily best practice to have multiple routes for the same page. But sometimes it's unavoidable. You'll notice if you try accessing the page in your browser, it'll show the URL `/about-me` at the top as it shows the content of the `about` route. 

Another option could be to use a `redirect`. Maybe you want users to be able to use the `about-me` route to get to the about page. But you don't want that route to show up in the URL. Maybe for SEO-reasons you want to make sure to only have a single route for your page.
...
So far we have defined only routes for the HTTP GET method. HTTP is a standard to help servers and clients communicate in a uniform way. There are [many different HTTP methods](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods). The most common ones are `GET` and `POST` (closely followed by `PUT`, `PATCH`, and `DELETE`). 
...
2. I used three single quotes to define the beginning and end of the string (this would also work with three double quotes). This is a standard way in Python to allow defining strings that have multiple lines in them. This allows me to split the HTML to two lines and keep my code more readable.

Try out different HTML tags and view the results in the web browser.

## Executing Backend Code

Up until now everything we've done was just return some text and HTML. You may wonder what we even need a web server for. Everything we've built would have been done much easier with a static websites built of simple HTML files. No need to have Python or Flask involved. And you would be right! So far we have not really done anything you'd server-code for. As you plan your web project, this is important to keep in mind. 
...
Sometimes, you want to define routes that are dynamic. Maybe you want to allow users to define routes for some pages of your app. Or maybe you have a blog with many articles. Imagine, you have a blog with a thousand articles and have to define every article route by hand using a decorator method and function. 
...
Let's say I want to have a cookies web shop. Users should be able to see an overview page with all cookies available on my website. Additionally, I want to have an individual page for each type of cookie. This is what I imagine in terms of routes:
...
The first route woudl list all the cookies. The following routes are examples for different types of cookies I want to sell individually. Each cookie should have their own page. 
...
To define a variable in a route you just write it within `<>` angle brackets and add it as parameter to your function. Let's define a cookie route:
...
Notice, the `/cookies/` part of the URL is still a set string. The only thing dynamic is whatever followas after in the place where you wrote `<slug>`. It's important that whatever you write in the angle brackets `<>` you also add as parameter to the function of the route below. 

"slug" is usually referred to as a string identifyer of a page that is formated in a URL-friendly way. So for example, "chocolate-chip" is URL fiendly because it doesn't contain special character or spaces and is all lower case. `mycoocieshop.com/cookies/Chocolate Chip` would **not** be URL-friendly. Therefore it's not considered a "slug". You can name the variable whatever you want. I just chose "slug" because it describes what I expect the user to type in here. 
...
Now, save everything and try accessing [127.0.0.1:5000/cookies/chocolate-chip](http://127.0.0.1:5000/cookies/chocolate-chip). Your web browser should return the Python dictionary as is. That's obviously not very pretty. So let's adjust that a little bit. Maybe display the name and price: 
...
To make our program even better and avoid errors, we could even add some logic that checks whether a cookie with the given slug exists. if it doesn't, we should return an error message instead: 
...
Now, to access an individual cookie dictionary you'd write something like this: `cookies_data[0]`. To access the price of the first cookie, you'd write `cookies_data[0]['price']`. Notice, how you need a integer (instead of a string) to access the item in a list. 

Flask gives you some extra tools to work with other data types such as integers. All you need to do is to define the variable type in the route using a colon: `@app.route('/cookies/<int:id>')`. In this example, you define the variable called `id` to be of the type `int` - which stands for integer. 

To then use that variable, your route could look like this: 
...
What you have learned in this exercise gives you a lot of tools and power to build a full website with different pages. You can even build a website with different routes. 
...
1. Come up with a data collection of your choice. It could be cookies, meals, todos, blog posts, what ever you can think of. 
