...
In this exercise, you'll built upon what you have learned previously about databases and models. One key element of relational databases are **relationships** between data sets - as the name suggests 🤯.
...
Depdending on whether your database is a SQL or NoSQL database, the data structures for those relationships could look very differently (but don't necessarily have to). This exercise will focus on relational (SQL) databases.
...
The second parameter defines that this will be a **foreign key**. Note, that the parameter of `db.ForeignKey()` is a string with the model name of the `Author` model in **lower case** plus `.id` as the name of the column, that represents the foreign key is `id`. It can be confusing and cause mistakes easily. So make sure you pay attention to when you need to user upper case or lower case.

_**Side note**: If you want to make sure a relationship always exists, you van add as a third parameter `nullable=False`. See more [details here](https://flask-sqlalchemy.palletsprojects.com/en/2.x/models/#one-to-many-relationships). What this will do is it'll make sure an `author_id` is always set and it will never be possible to create an `Article` record without a corresponding `author_id`._
...
You can see, next to a bunch of fields to store the address, we added a field `order_id`. That's the column referring back to our order using a foreign key. In a one-to-one relationship it actually doesn't really matter which of the two records keeps the foreign key. One way to choose the model with the foreign key could be think about which one could exist without the other. In our example, we might have an order without an address. But we really won't have an address without an order. Therefore, we added the `order_id` as foreign key to the `Address` model. 
...
Don't actually do that in our project. Instead, we're going to move on to the last type of relationships:
...
The two most important columns are `cookie_id` and `order_id`. If you create a many-to-many relationship, those are the only two column you need. But because we want to also store how many cookies someone ordered we added a third column that stores that exact number. 
...
`lazy='subquery'` is just a more efficient way in handling the relationship here. Finally, the `backref` is a little more complex as we specifically set the `orders` hereto be accessible through a single cookie instance. 
